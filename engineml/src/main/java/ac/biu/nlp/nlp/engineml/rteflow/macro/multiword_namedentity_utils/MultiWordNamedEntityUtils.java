...
import ac.biu.nlp.nlp.engineml.operations.rules.lexicalmw_utils.LexicalRuleInTreeFinder;
import ac.biu.nlp.nlp.engineml.operations.rules.lexicalmw_utils.ListNodesToTree;
				...
				for (List<BasicNode> neList : vsmNeInTree.get(lhsNode))
				{
					if (neList.size()>1)
					{
						...
						if (AbstractNodeUtils.treeToSet(rhsTree).size()==neList.size())
						{
							...
							Rule<Info, BasicNode> rule = new Rule<Info, BasicNode>(ruleLhs, rhsTree, mapNodes);
