...
/**
 * Efficient implementation of re-attaching a node to the reduced graph.
 * Each re-attach take linear time by using dynamic programming
 * @author User
 *
 */
public class EfficientlyCorrectGreedyTreeLearner extends GreedyScoreTreeLearner {

	private Logger logger = Logger.getLogger(EfficientlyCorrectGreedyTreeLearner.class);
	
	public EfficientlyCorrectGreedyTreeLearner(double edgeCost) {
		super(edgeCost);
	}

	public EfficientlyCorrectGreedyTreeLearner(NodeGraph ioGraph,ScoreModel iLocalModel, double edgeCost) throws Exception {
