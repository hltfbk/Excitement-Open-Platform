...
public class EfficientlyCorrectHtlTreeLearner extends HighToLowTreeLearner{

	private Logger logger = Logger.getLogger(EfficientlyCorrectHtlTreeLearner.class);
	public EfficientlyCorrectHtlTreeLearner(NodeGraph ioGraph,MapLocalScorer iLocalModel, double edgeCost) throws Exception {
		super(ioGraph, iLocalModel, edgeCost);
	}

	public EfficientlyCorrectHtlTreeLearner(double edgeCost) {
		super(edgeCost);
	}
	...
	private void reattachNode(RelationNode removedNode) throws Exception {
		...
		DirectedOneMappingOntologyGraph graphCopy =  graph.reduceGraph();
